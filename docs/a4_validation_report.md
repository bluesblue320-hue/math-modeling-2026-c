...
| **M3** | 问题 1 LP 固有 | LP 存在多重最优解：A4 独立最优解与 A3 解目标相同但逐时段 dispatch 不同（同价时段间 grid/charge 重排，最大差 ~2000 kW）。A3 解为**有效最优解**，非缺陷；记录以备后续若需唯一化时采用 model_spec §7 方案 B（固定 C\* 后最小化吞吐量）。 | 无 |
