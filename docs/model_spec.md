...
> **时间标签映射（唯一依据：`docs/time_mapping_decision.md`，当前 `STATUS: DECIDED`）**：采用 **Mapping A**——附件 1 的时间标签是其对应 10 min 区间的**结束时刻（右端点）**。标签 $\tau = t\Delta t$（如 00:10、10:10、`0:00+1`）对应时段 $t$，即第 $(t+1)$ 行数据对应时段 $t$（§8 示例据此成立）。
> 该口径是为**完整覆盖当天 00:00–24:00、并与首尾储电量、表 2 边界保持一致**所作的**建模解释**，**非题目明文定义**（题目仅定义 `0:00+1` = 当天 24:00）。候选「标签 = 区间开始时刻」已否决。详细证据、边界处理与完整映射见该文件。
...
> **注（映射已定）**：上表 $t$ 值按 `docs/time_mapping_decision.md`（`STATUS: DECIDED`，**Mapping A**）确定：时段 $t$ 覆盖 $(t-1)\Delta t$–$t\Delta t$，输入标签为其**结束时刻**，附件 1 `source_row = t + 1`。例：`10:00–10:10`（$t=61$）取**附件 1 第 62 行**（标签 `10:10`）；其余五个时段依次取第 74、86、98、110、122 行。
...
> 表 2 的 4 h 区间端点（0:00 与 24:00）由题目固定；上列「块 ↔ 时段号」对应与 `docs/time_mapping_decision.md`（Mapping A）一致。

**result1.xlsx**：
- "计划购电量"表 A2:A145 为 144 个 10 min 标签，B 列填 $Q^{\text{buy}}_t$（kWh）。模板印示时间段相对物理区间**整体后移一格**（无当天 `00:00-00:10`，多含次日 `0:00+1-0:10+1`）；按 `docs/time_mapping_decision.md`（`STATUS: DECIDED`，Mapping A）约定 `output_row = model_t + 1`（电子表格行号），并须按该文件第 3.3 节显式修正/说明标签，**不得静默平移**。
...
1. **时间映射前置校验**：求解前，以 `docs/time_mapping_decision.md` 为准（当前 `STATUS: DECIDED`、`BLOCK_A2: FALSE`，Mapping A）。严格按该文件 `source_row → source_label → model_t → physical_interval → output_row` 映射执行，不得偏离；输入侧 `source_row = model_t + 1`，输出侧 `output_row = model_t + 1` 并按该文件第 3.3 节处理模板标签。
...
> 主模型仍为 **LP**；默认仅在验证发现同时充放电时按需启用方案 B。本次工程补充允许显式请求 `representative_optimum=True`，即使无同时充放电也可选择代表解。其第二阶段数学口径仍为固定 $C_{\text{buy}} = C^{*}_{\text{buy}}$ 后最小化吞吐量；数值实现用 $|C_{\text{buy}}-C^{*}_{\text{buy}}|\le10^{-6}$ 元，另允许 $10^{-8}$ 元求解浮点误差。代表解单独保存，不替换主结果、不加入退化成本。最优购电费用确定，但对应调度轨迹可能存在多重最优解；最小吞吐量也不保证轨迹唯一。
...
| 编号 | 假设 | 说明/依据 |
| --- | --- | --- |
| H1 | **时间标签口径：采用 Mapping A（标签 = 区间结束 / 右端点）**，$\text{model\_t}=t \leftrightarrow \big[(t-1)\Delta t,\ t\Delta t\big]$，附件 1 `source_row = t + 1`；唯一依据 `docs/time_mapping_decision.md`（`STATUS: DECIDED`）。该口径为**建模解释**（为完整覆盖当天 00:00–24:00、与 0:00/24:00 储电量及表 2 边界一致），**非题目明文定义**；候选「标签 = 区间左端点」已 REJECTED。 | A1、A2；输出模板整体后移一格，须按该文件第 3.3 节显式处理，不得静默平移 |
...
## 13. 工程补充留痕（2026-09-11）

依据用户本轮 Q1 工程修复要求，第 9 节补充默认关闭的代表解选择及数值容差说明。主目标、物理约束、H3 默认效率与 Mapping A 均未改动。效率参数化仅服务第 3 节已允许的解释敏感性分析，主论文仍取双向各 0.90。

已同步检查 `docs/problem_spec.md`（第 7 节 A4 效率解释未决项、A13 数值精度）及 `docs/time_mapping_decision.md`（第 3.3 节导出标签修正规则）：无冲突，无须修改两文件。输出标签修正只应用于官方模板的结果副本，原始模板不变。
